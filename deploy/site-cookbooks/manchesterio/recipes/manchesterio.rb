...
supervisor_service "manchesterio" do
  ...
  command "#{node.manchesterio.root}/bin/manchesterio/bin/gunicorn manchesterio.app:app -b 127.0.0.1:8000 -e DEBUG=#{node.manchesterio.debug} -e API_BASE_URL=#{node.manchesterio.api_hostname} #{extra_args}"
