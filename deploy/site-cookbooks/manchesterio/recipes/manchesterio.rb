...
supervisor_service "manchesterio" do
  ...
  if node.manchesterio.debug
    ...
    command "#{node.manchesterio.root}/bin/manchesterio/bin/gunicorn manchesterio.app:app -b 127.0.0.1:8000 -e API_BASE_URL=#{node.manchesterio.api_hostname} "
  end
  environment "PYTHONPATH" => "#{node.manchesterio.root}/manchesterio", 'API_BASE_URL' => node.manchesterio.api_hostname
end
